...
| `Dockerfile.cased`, `compose.yaml`, `.dockerignore`, `.env.example`, `requirements*.txt` | [GNU AGPL v3](LICENSES/AGPL-3.0.txt) | `AGPL-3.0-only` |
| `mcp/`, `bin/case`, `web/`, `tests/` | [MIT](LICENSES/MIT.txt) | `MIT` |
| `Dockerfile.ui`, `case-mcp.json` | [MIT](LICENSES/MIT.txt) | `MIT` |
...
Each Python, shell, and JavaScript source file carries an
`SPDX-License-Identifier` header. The table above covers build and configuration
...
signed once by adding your GitHub username to [`CLA-SIGNERS`](CLA-SIGNERS) in
your first pull request. You keep your copyright; the project gets the right to
sublicense. That grant is precisely what makes the dual licensing above
possible, and it cannot be reconstructed later, which is why it is asked for up
front. See [CONTRIBUTING.md](CONTRIBUTING.md).
