...
def test_stdio_is_the_default():
    assert _helpers.load_case_mcp().HTTP is False  # unset env → every existing flow untouched


def test_http_mode_defaults_to_loopback():
    m = _helpers.load_case_mcp(CASE_MCP_HTTP="1", CASE_MCP_PORT="8899")
...
def test_http_mode_honours_bind_env():
    m = _helpers.load_case_mcp(CASE_MCP_HTTP="1", CASE_MCP_BIND="0.0.0.0")
    assert m.mcp.settings.host == "0.0.0.0"


def test_http_app_serves_mcp_path():
    m = _helpers.load_case_mcp(CASE_MCP_HTTP="1")
...
def _mcp_statuses(requests_, **env):
    ...
    m = _helpers.load_case_mcp(CASE_MCP_HTTP="1", CASE_MCP_BIND="0.0.0.0", **env)
